...
void read_xml_element(const char* file_name, const char* element_name) {
    ...
    //3.获取子节点信息
    tinyxml2::XMLElement* childNodeStu = xmlRoot->FirstChildElement(element_name);
    if (childNodeStu == NULL) {
        printf("No element %s\n", element_name);
        return;
    }
    const char* contentStu = childNodeStu->GetText();
    printf("element = %s, value = %s\n", element_name, contentStu);
}
